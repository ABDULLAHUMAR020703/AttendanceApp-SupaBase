...
export const createLeaveRequest = async (employeeId, leaveType, startDate, endDate, reason = '', isHalfDay = false, halfDayPeriod = null, category = null) => {
  try {
    ...
    const requestData = {
      ...
      employee_name: employee.name || employee.username || null,
  ...
  try {
    ...
    return (requests || []).map(req => {
      ...
      return {
        ...
        employeeName: employee?.name || employee?.username || req.employee_name || req.employee_id,
        employeeUsername: employee?.username || req.employee_username || req.employee_id,
        leaveType: req.leave_type,
        startDate: req.start_date,
        endDate: req.end_date,
        days: req.days,
